...
> **How to use:** When generating a Sliders component, pick ONE option from EACH axis below to create a unique combination. Never use the same combination twice.

### 📐 Layout Axis
1. Horizontal range slider
2. Vertical range slider
3. Dual-thumb range selector
4. Step/discrete slider with markers
5. Circular/radial slider

### 🎨 Surface Axis
1. Track with brand-color filled portion
2. Glass track with glowing thumb
3. Minimal thin line with dot thumb
4. Thick track with embedded value label
5. Gradient track showing value spectrum

### ⚡ Motion Axis
1. Thumb follows with spring physics
2. Value tooltip pops up on drag
3. Track fill animates on programmatic change
4. Step markers pulse when passed
5. Thumb has glow that intensifies on drag

### 🎭 Mood Axis
1. System — clean, standard range input
2. Audio — volume slider, waveform track
3. Pricing — value-based, dollar amount thumb
4. Color — hue/saturation spectrum track
5. Custom — emoji markers, playful positions

### 🧩 Composition Axis
1. Volume/brightness control
2. Price range filter
3. Form numeric input alternative
4. Color picker hue/sat control
5. Settings preference adjustment
...
### Recipe 1: Glow Range
horizontal slider with dark track. Filled portion in brand-color. Thumb is circular with box-shadow glow (brand-color). Glow intensifies on drag. Value tooltip appears above thumb during drag with spring pop. Track has subtle gradient from muted to brand.

### Recipe 2: Step Slider
discrete slider with visible step markers (dots on track). Thumb snaps between steps with spring animation. Active step marker enlarges. Value labels below markers. Current value displayed prominently above. Used for plan tier selection or similar.

### Recipe 3: Dual Range
two thumbs for min/max range. Filled portion between thumbs has gradient. Thumbs can't cross. Value labels on each thumb. Track shows full range context. Used for price, age, or date range filters. Both thumbs have spring physics.

### Recipe 4: Audio Fader
vertical slider styled as audio mixer fader. Tall thin track with large horizontal thumb grip. dB markings along side. Peak indicator LED stack above. Thumb casts shadow. Drag with smooth response. Used for creative/audio UIs.
...
Load these companion blueprints when building with Sliders:
