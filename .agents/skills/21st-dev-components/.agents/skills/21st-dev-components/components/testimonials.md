...
> **How to use:** When generating a Testimonials component, pick ONE option from EACH axis below to create a unique combination. Never use the same combination twice.

### 📐 Layout Axis
1. Card grid (2-3 columns)
2. Single testimonial spotlight
3. Carousel slider
4. Masonry wall
5. Quote + avatar inline strip

### 🎨 Surface Axis
1. Glass cards on gradient background
2. Solid dark cards with quote marks
3. Outlined cards with avatar header
4. Full-bleed image background with overlay
5. Clean white cards with shadow

### ⚡ Motion Axis
1. Cards stagger-in on scroll
2. Carousel auto-rotates with crossfade
3. Quote marks float in before text
4. Avatar pops in with spring
5. Star rating fills in sequentially

### 🎭 Mood Axis
1. Premium — dark, large quote, subtle, one at a time
2. Social proof — many small cards, volume over size
3. Personal — large avatar, name/role, detailed quote
4. Enterprise — company logos, role titles, formal
5. Creative — photo backgrounds, overlay text, editorial

### 🧩 Composition Axis
1. Homepage social proof section
2. Product page reviews
3. Landing page trust block
4. Case study page header
5. About page team testimonials
...
### Recipe 1: Glass Wall
3-column masonry grid of glass testimonial cards. Each: large open-quote glyph (brand-color, 48px), quote text, avatar + name + role footer. Cards stagger-in on scroll viewport enter. Hover: card lifts with shadow bloom. Star rating below quote. Varied card heights create masonry effect.

### Recipe 2: Spotlight Rotator
single large testimonial, centered. Auto-rotates every 6s with crossfade transition. Large avatar (64px), name, role, company logo. Quote in text-xl italic. Navigation dots below. Pause on hover. Brand-color accent line above quote. Premium, focused.

### Recipe 3: Logo Strip
horizontal scrolling strip of company logos. Below each logo: one short testimonial sentence. Infinite ticker scroll. Hover pauses and shows full testimonial in popover above. Clean, volume-focused social proof.

### Recipe 4: Video Testimonials
grid of cards where each contains a video thumbnail with play button overlay. Click: modal opens with video player. Name + role below video. Play button has brand-color glow. Cards stagger-in. Engaging, high-trust format.
...
Load these companion blueprints when building with Testimonials:

- `components/avatars.md` — Avatars
- `components/cards.md` — Cards
- `components/carousels.md` — Carousels
- `components/clients.md` — Clients
