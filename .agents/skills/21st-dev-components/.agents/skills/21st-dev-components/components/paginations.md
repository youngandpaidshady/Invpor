...
> **How to use:** When generating a Paginations component, pick ONE option from EACH axis below to create a unique combination. Never use the same combination twice.

### 📐 Layout Axis
1. Numbered page buttons row
2. Previous/Next only
3. Infinite scroll with loading trigger
4. Load more button
5. Cursor-based endless feed

### 🎨 Surface Axis
1. Outlined page number buttons
2. Active page with brand-color fill
3. Minimal text-only page numbers
4. Glass pill-shaped page buttons
5. Floating pagination bar

### ⚡ Motion Axis
1. Active page indicator slides between numbers
2. Page transition: content crossfades
3. Load more: spinner + new items stagger in
4. Button hover: scale + micro-lift
5. Infinite scroll: skeleton items appear then resolve

### 🎭 Mood Axis
1. Clean modern — pill buttons, active indicator
2. Minimal — text numbers only, underline active
3. Table pagination — compact, rows-per-page select
4. Content feed — load more, endless scroll
5. Dashboard — detailed: showing X of Y results

### 🧩 Composition Axis
1. Table data pagination
2. Blog post list pagination
3. Search results pages
4. Product catalog pagination
5. Image gallery pages
...
### Recipe 1: Sliding Indicator
row of page number buttons. Active page has brand-color background pill that slides between positions (layoutId spring animation). Hover: button lifts with shadow. Ellipsis (…) for truncated ranges. Previous/Next arrows at edges. Clean, modern, satisfying.

### Recipe 2: Infinite River
no visible pagination controls. Scroll near bottom triggers loading. Skeleton placeholder items appear, then resolve to real content with crossfade. Loading spinner at bottom during fetch. Smooth, social-media-style feed.

### Recipe 3: Glass Bar
fixed floating glass bar at bottom-center. Contains: left arrow, page numbers, right arrow. Active page highlighted. Bar has backdrop-blur + border-white/10. Fades in on scroll-down, fades out at top. Compact, elegant.

### Recipe 4: Progressive Load
'Load More' button centered below content. Click: button shows spinner, then new items stagger-in below existing content. Button moves down. Count text: 'Showing 20 of 156'. Keeps scroll position. Clean, user-controlled.
...
Load these companion blueprints when building with Paginations:
