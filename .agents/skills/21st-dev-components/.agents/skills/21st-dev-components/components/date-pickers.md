...
> **How to use:** When generating a Date Pickers component, pick ONE option from EACH axis below to create a unique combination. Never use the same combination twice.

### 📐 Layout Axis
1. Single date input with calendar dropdown
2. Date range dual input
3. Inline always-visible calendar
4. Week picker strip
5. Month/year only selector

### 🎨 Surface Axis
1. Glass dropdown on dark background
2. Outlined calendar with brand-color selected
3. Minimal flat with hover highlights
4. Card-elevated with shadow
5. Frosted pill input with glass dropdown

### ⚡ Motion Axis
1. Dropdown springs in with scale from input
2. Date selection pops with micro-bounce
3. Month transition slides left/right
4. Input text morphs to formatted date
5. Calendar cells stagger-in by row

### 🎭 Mood Axis
1. Dashboard — compact, data-focused
2. Booking — range-centric, date highlighting
3. Minimal — iOS-style wheel/picker feel
4. Enterprise — ISO format, week numbers visible
5. Playful — emoji for special dates, colorful

### 🧩 Composition Axis
1. Form field date input
2. Filter bar date range
3. Booking date selector
4. Event creation dialog
5. Dashboard date filter
...
### Recipe 1: Glass Date
frosted-glass input pill. Click opens glass dropdown calendar with backdrop-blur-xl. Selected date has brand-color ring + glow. Month arrows spring animate. Calendar springs in from input anchor point with scale. Today has pulsing dot.

### Recipe 2: Range Sweep
dual date input for start/end. Calendar shows range with gradient fill between dates. Dragging either end updates in real-time. Range dates glow subtly. Input displays formatted 'Jan 12 - Jan 18'.

### Recipe 3: Inline Planner
always-visible calendar embedded in a card. Selected date highlights with brand fill + lift. Events shown as colored dots below date numbers. Navigate months via swipe gesture or arrows with slide transition.

### Recipe 4: Wheel Selector
iOS-style scrolling drum picker for day/month/year in three columns. Snap scroll physics. Selected row centered and enlarged. Backdrop fades above and below center. Haptic-style visual feedback.
...
Load these companion blueprints when building with Date Pickers:

- `components/calendars.md` — Calendars
